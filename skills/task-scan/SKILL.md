...
> **플러그인 내장판(1.54.0~)**: 같은 규칙표가 core `task-scan.ts`로 플러그인에 내장되어 `할 일 자동 수집` 명령·로드 시 자동 실행(설정 옵트인)으로도 돈다. sourceKey 규약이 동일해 어느 쪽이 먼저 수집해도 중복되지 않는다. 이 스킬은 옵시디언이 닫혀 있는 시간대의 예약 실행(헤드리스) 경로로 유지한다. **규칙표를 고치면 core 모듈과 이 문서를 함께 고칠 것.**
