...
1. [docs/PRODUCT.md](docs/PRODUCT.md) — 목표·비목표·아키텍처 3원칙(노트=진실/원장=계산/과거=불변)·데이터 모델(kind 24종)·LLM 협업 모델(§9-1)·로드맵(§10)
...
```bash
npx tsc -noEmit -skipLibCheck && npm test && npm run lint   # 테스트 150+개·린트, 전부 통과 필수
```
