...
test("시간표 변경을 노트 내용에 추가·교체·제거한다", () => {
  ...
  let parsed = parseBaseTimetable(timetableFile, { "class-management": "timetable", schoolYear: "2026", semester: "1학기" }, added);
  ...
  parsed = parseBaseTimetable(timetableFile, { "class-management": "timetable", schoolYear: "2026", semester: "1학기" }, replaced);
  assert.equal(parsed.overrides.length, 2);
  assert.equal(
  ...
  parsed = parseBaseTimetable(timetableFile, { "class-management": "timetable", schoolYear: "2026", semester: "1학기" }, removed);
...
test("기준 교시를 넘는 변경·행사는 그날 교시를 확장한다", () => {
  ...
  const parsed = parseBaseTimetable(timetableFile, { "class-management": "timetable", schoolYear: "2026", semester: "1학기" }, extended);
  ...
  const parsedReverted = parseBaseTimetable(timetableFile, { "class-management": "timetable", schoolYear: "2026", semester: "1학기" }, reverted);
...
test("시간표 변경이 행사보다 우선한다", () => {
  ...
  const parsed = parseBaseTimetable(timetableFile, { "class-management": "timetable", schoolYear: "2026", semester: "1학기" }, withOverride);
...
test("삭제 표식은 그날 해당 교시를 없앤다", () => {
  ...
  const parsed = parseBaseTimetable(timetableFile, { "class-management": "timetable", schoolYear: "2026", semester: "1학기" }, removed);
  const monday = resolveDay(calendar, parsed, "2026-05-04");
  ...
  const parsedRestored = parseBaseTimetable(timetableFile, { "class-management": "timetable", schoolYear: "2026", semester: "1학기" }, restored);
  ...
  const parsedReassigned = parseBaseTimetable(timetableFile, { "class-management": "timetable", schoolYear: "2026", semester: "1학기" }, reassigned);
...
test("시간표 변경 절이 없으면 새로 만든다", () => {
  ...
  const parsed = parseBaseTimetable(timetableFile, { "class-management": "timetable", schoolYear: "2026", semester: "1학기" }, added);
  assert.equal(parsed.overrides.length, 1);
...
test("스캐폴드 시간표를 되읽을 수 있다", () => {
  ...
  const parsed = parseBaseTimetable(timetableFile, { "class-management": "timetable", schoolYear: "2026", semester: "2학기" }, markdown);
  assert.equal(parsed.periods, 6);
  assert.equal(parsed.grid[5][0], "－");
});

test("판별자가 다른 노트는 기초시간표로 파싱하지 않는다", () => {
  const markdown = baseTimetableMarkdown("2026", "2학기", "우리 반", [5, 6, 5, 6, 5]);
  assert.equal(parseBaseTimetable(timetableFile, { "class-management": "subject-progress" }, markdown), null);
  assert.equal(parseBaseTimetable(timetableFile, {}, markdown), null);
});
