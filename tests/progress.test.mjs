...
test("안전교육 합산은 행 단위 1회 — 중복 태그·#안전교육 직접 태그 처리", async () => {
  const { crossCurricularAudit } = await loadTypeScriptModule("../packages/core/src/progress.ts");
  const table = (semester, subject, rows) => ({ file, schoolYear: "2026", semester, subject, rows });
  const rows = crossCurricularAudit(
    {
      "2학기": [
        table("2학기", "체육", [
          { ...makeRow(1, "복합 안전", 2), note: "#생활안전 #교통안전" },
          { ...makeRow(2, "직접 태그", 1), note: "#안전교육" },
          { ...makeRow(3, "일반", 1), note: "#보건교육" }
        ])
      ]
    },
    [{ subject: "안전교육", hours1: 0, hours2: 0, hours: 51 }],
    { today: "2026-12-31", weekStart: "2026-12-28", weekEnd: "2027-01-01" }
  );
  const safety = rows[0];
  assert.equal(safety.plannedYear, 3, "복합 태그 2시간은 1회(2) + 직접 태그 1 = 3 (중복 누계 없음)");
  assert.equal(rows.find((row) => row.tag === "생활안전").plannedYear, 2, "세부 영역 행은 각자 계상");
  assert.equal(rows.find((row) => row.tag === "교통안전").plannedYear, 2);
  assert.ok(!rows.some((row) => row.name === "#안전교육"), "#안전교육은 기준 행에 흡수 — 별도 행 없음");
  assert.equal(rows.find((row) => row.tag === "보건교육").plannedYear, 1, "안전 외 태그는 무관");
});
