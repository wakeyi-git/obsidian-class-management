...
test("전량 이관 단원의 plannedHours 0이 왕복 보존된다", () => {
  const unit = emptyCurriculumUnit(curriculumSettings);
  unit.unitName = "전량 이관";
  unit.plannedHours = 0;
  const markdown = curriculumUnitMarkdown(unit, curriculumSettings);
  assert.match(markdown, /plannedHours: 0/);
  const parsed = parseCurriculumUnit(
    { path: "u.md", basename: "u", stat: { ctime: 1 } },
    { "class-management": "curriculum-unit", curriculumUnitId: unit.id, unitName: "전량 이관", plannedHours: 0 },
    markdown
  );
  assert.equal(parsed.plannedHours, 0);
  // 값이 없으면 기존처럼 기본 1
  const missing = parseCurriculumUnit(
    { path: "u.md", basename: "u", stat: { ctime: 1 } },
    { "class-management": "curriculum-unit", curriculumUnitId: unit.id, unitName: "전량 이관" },
    markdown
  );
