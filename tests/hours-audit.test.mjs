...
test("구형 2열 기준 시수 표를 파싱하고 구분을 이름으로 추정한다", () => {
  ...
  assert.deepEqual(standard.entries, [
    { subject: "국어", hours: 204, category: "교과" },
    { subject: "창체(자율)", hours: 34, category: "창체" }
  ]);
});
...
test("구분 열이 있는 3열 표를 파싱하고 빈 구분은 위 행을 승계한다", () => {
  ...
  assert.deepEqual(standard.entries, [
    { subject: "국어", hours: 198, category: "교과" },
    { subject: "디지털 놀이터", hours: 29, category: "교과" },
    { subject: "창체(자율)", hours: 73, category: "창체" }
  ]);
});
...
test("스캐폴드는 구분 열과 학교자율시간 안내를 담는다", () => {
  ...
  assert.match(markdown, /\| 구분 \| 교과·영역 \| 연간 기준 시수 \|/);
...
function makeStandard() {
  return {
    ...
    tolerancePercent: 20,
    entries: [
      { subject: "국어", hours: 100, category: "교과" },
      { subject: "수학", hours: 100, category: "교과" },
      { subject: "창체(자율)", hours: 40, category: "창체" },
      { subject: "창체(진로)", hours: 10, category: "창체" }
    ]
  };
}
